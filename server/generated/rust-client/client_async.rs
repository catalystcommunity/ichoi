...
impl<T: AsyncTransport> LibraryAsyncClient<T> {
    ...
    pub async fn export_chunk(&self, req: ExportChunkRequest) -> Result<ExportChunk, ClientError> {
        let csil_resp = self
            .transport
            .call(
                "LibraryService",
...
impl<T: AsyncTransport> AdminAsyncClient<T> {
    ...
    pub async fn list_trusted_identities(
        &self,
        req: Page,
    ...
    pub async fn begin_import(
        &self,
        req: BeginImportRequest,
    ) -> Result<BeginImportResult, ClientError> {
        let csil_resp = self
            .transport
            .call(
                "AdminService",
    ...
    pub async fn import_chunk(&self, req: ImportChunkRequest) -> Result<Ok, ClientError> {
        let csil_resp = self
            .transport
            .call(
                "AdminService",
    ...
    pub async fn cancel_import(&self, req: CancelImportRequest) -> Result<Ok, ClientError> {
        let csil_resp = self
            .transport
            .call(
                "AdminService",
...
pub struct ChangeAsyncClient<T: AsyncTransport> {
    #[allow(dead_code)]
    transport: T,
}

impl<T: AsyncTransport> ChangeAsyncClient<T> {
    pub fn new(transport: T) -> Self {
        Self { transport }
    }
